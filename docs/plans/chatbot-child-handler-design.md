...
**Two corrections from the first draft of this doc, both found while building it, the second one live:**

1. The plan above originally said the synthetic question could reuse `Question`/`ConversationAnswer` completely unchanged, needing "no special-casing at confirm time, no new column on `chat.conversation`." That didn't hold: `chat.conversation_answer.question_id` has a real foreign key to `form.question.question_id`, and the picker's synthetic question has no corresponding row there (it's built client-side, not part of any form definition) — so inserting a normal `ConversationAnswer` for it fails the FK at the database level, not just in theory.

2. The first fix for #1 tried routing the picker step through `chat.conversation.current_question_id` using a small set of fixed, made-up UUIDs (one per parent type). That seemed safe because the SQLAlchemy model declares no `ForeignKey()` on that column — but the *live* table has one anyway (`fk_chat_conversation_current_question` → `form.question.question_id`) that the Python model simply never declared. First real farmer-facing test hit this immediately: `ForeignKeyViolationError` the moment `start_conversation` tried to insert a made-up UUID there. Checking the model isn't the same as checking the database.

What was actually built, once both were found: the picker step leaves `current_question_id` `NULL` (which the FK already permits — the same state `AWAITING_CONFIRMATION` already uses) and tracks which picker is pending inside `chat.conversation.parent_answer` itself, as `{"pending_kind": "farm_activity"}`, until a real pick resolves it to `{"field_name": ..., "value": ...}`. `confirm_conversation` merges the resolved shape into the submission payload alongside the form's real answers, using the same `{field_name: value}` construction every other answer already goes through — so the field_name trick itself still holds, just via one plain JSONB column instead of the `Question`/`ConversationAnswer` machinery. No `form.question` row, real or fake, is needed anywhere. `database/migrations/V13__conversation_parent_answer.sql` (the `parent_answer` column itself) didn't need to change for this second fix — only how its JSON shape is used.

**Verified live**, not just by test: a real submission through `farm_activity_fertilizer` on 2026-08-19 went end to end — picker showed the correct farm-scoped activity, the real form questions followed, and Go inserted the row with the picked `farm_activity_id`. The one remaining bug that surfaced on that same run was unrelated to the picker itself: `src/tasks/client.py`'s `submit_task` used httpx's 5s default timeout, and Go's first-ever call against a newly-unblocked handler's table took 6.6s (a one-time cold-cache cost, same shape as `get_form()`'s own BE-5 note) — so the chatbot reported failure and told the farmer to retry, even though Go had already succeeded, which risked a duplicate row on retry (`dissectAnswer` has no idempotency guard). Fixed by giving `submit_task` the same 30s timeout `get_form()` already uses, for the same reason.
