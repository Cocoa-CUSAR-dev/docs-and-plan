...
**Proposed design for CB-6** (not built, logged for whenever the team picks it up): full writeup at [Designing the 5 Blocked Handlers for the Chatbot](/docs/plans/chatbot-child-handler-design). Short version — before asking the real questions on one of the 5 child-row forms, synthesize one more guided-flow question asking the farmer to pick from their own recent entries of the right parent type, reusing the existing engine unchanged (the picked ID rides through as a normal field, keyed on the real destination column name). This surfaced a real prerequisite gap while designing it: none of `agriculture.farm_activity`/`collection.harvest`/`processing.batch` record who created the row at all today, so "the farmer's own recent entries" can't be queried without a small migration first (`created_by_user_id` on those 3 tables). Also flags one open product question the design doesn't resolve: whether a processing batch's "owner" should be scoped to whoever created it, or to anyone working at that processing station.
